#!/usr/bin/env python3
"""
METMC OS Bridge Client — Linux-side daemon

Connects to the Android bridge service via Unix domain socket at
/run/metmc/bridge.sock and exposes hardware APIs as D-Bus services
...
Usage:
    metmc-bridge [--socket /run/metmc/bridge.sock] [--debug]
"""
...
DEFAULT_SOCKET = "/run/metmc/bridge.sock"
RECONNECT_DELAY = 3  # seconds between reconnection attempts
REQUEST_TIMEOUT = 10  # seconds

logger = logging.getLogger("metmc-bridge")
...
class METMC OSBridge:
    ...
    def start(self):
        ...
        logger.info("METMC OS bridge client starting...")
        ...
        logger.info("METMC OS bridge client stopped")
...
def main():
    parser = argparse.ArgumentParser(description="METMC OS bridge client daemon")
    parser.add_argument(
    ...
    bridge = METMC OSBridge(args.socket)
    bridge.start()
