...
class T3PrivateBrowserView(private val context: Context) : LinearLayout(context) {
    ...
    private fun build() {
        ...
        fun button(text: String, description: String, action: () -> Unit): Button =
        ...
        top.addView(button("‹", "Back") {
            if (browser.canGoBack()) browser.goBack()
        }, sizeParams(42))
        top.addView(button("›", "Forward") {
        ...
        address = EditText(context).apply {
            hint = "Search Yahoo or enter website"
            textSize = 14f
            setSingleLine(true)
            setTextColor(Color.WHITE)
            setHintTextColor(Color.rgb(145, 152, 164))
            setPadding(dp(12), 0, dp(12), 0)
            background = rounded(Color.rgb(38, 43, 53), dp(10))
        ...
        title = TextView(context).apply {
            text = "T3 Private Browser • Yahoo"
            textSize = 11f
            setTextColor(Color.rgb(160, 168, 182))
            setPadding(dp(12), 0, dp(12), 0)
            gravity = Gravity.CENTER_VERTICAL
        }
        ...
        status = TextView(context).apply {
            text = "Private session • cookies disabled"
            textSize = 11f
            setTextColor(Color.rgb(145, 152, 164))
            setPadding(dp(12), 0, dp(12), 0)
            gravity = Gravity.CENTER_VERTICAL
        }
        ...
        browser = WebView(context).apply {
            ...
            webViewClient = object : WebViewClient() {
                ...
                override fun onPageFinished(view: WebView, url: String) {
                    ...
                    title.text = view.title?.takeIf { it.isNotBlank() } ?: url
                    status.text = "Ready • Yahoo search"
                }

                override fun onReceivedError(view: WebView, request: WebResourceRequest, error: WebResourceError) {
                    if (request.isForMainFrame) {
                        title.text = "Page error"
                        status.text = error.description?.toString() ?: "Unable to load page"
                    }
                }
            }
            ...
            setDownloadListener(DownloadListener { url, _, _, _, _ ->
                try {
                    ...
                    Toast.makeText(context, "No app can handle this download", Toast.LENGTH_SHORT).show()
                }
            })
        }
    ...
    private fun sizeParams(width: Int) = LinearLayout.LayoutParams(dp(width), dp(44)).apply {
        marginStart = dp(2)
        marginEnd = dp(2)
    }
