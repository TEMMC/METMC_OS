...
class T3PrivateBrowserView(private val context: Context) : LinearLayout(context) {
    ...
    private fun build() {
        ...
        top.addView(button("⌂","Yahoo Home"){navigate(yahooHome)}, LinearLayout.LayoutParams(dp(42),dp(44)))
        address=EditText(context).apply { hint="Search Yahoo or enter website"; textSize=14f; setSingleLine(true); setTextColor(Color.WHITE); setHintTextColor(Color.rgb(145,152,164)); setPadding(dp(12),0,dp(12),0); background=rounded(Color.rgb(38,43,53),dp(10)); imeOptions=EditorInfo.IME_ACTION_GO }
        ...
        title=TextView(context).apply{text="T3 Private Browser • Yahoo"; textSize=11f; setTextColor(Color.rgb(160,168,182)); setPadding(dp(12),0,dp(12),0); gravity=Gravity.CENTER_VERTICAL}; addView(title,LinearLayout.LayoutParams(-1,dp(27)))
        status=TextView(context).apply{text="Private session • cookies disabled"; textSize=11f; setTextColor(Color.rgb(145,152,164)); setPadding(dp(12),0,dp(12),0); gravity=Gravity.CENTER_VERTICAL}
        browser=WebView(context).apply{setBackgroundColor(Color.WHITE)}
        browser.settings.apply { javaScriptEnabled=true; domStorageEnabled=true; databaseEnabled=true; loadsImagesAutomatically=true; builtInZoomControls=true; displayZoomControls=false; useWideViewPort=true; loadWithOverviewMode=true; mediaPlaybackRequiresUserGesture=false; cacheMode=WebSettings.LOAD_DEFAULT; allowFileAccess=false; allowContentAccess=false; setSupportZoom(true); javaScriptCanOpenWindowsAutomatically=false; setSupportMultipleWindows(false); userAgentString=userAgentString.replace("; wv","") }
        ...
        browser.webViewClient=object:WebViewClient(){
            ...
            override fun onPageFinished(view:WebView,url:String){address.setText(url);address.setSelection(address.text?.length ?: 0);title.text=view.title?.takeIf{it.isNotBlank()}?:url;status.text="Ready • Yahoo search"}
    ...
    private fun navigate(value:String){val input=value.trim();if(input.isEmpty())return;val url=when{input.startsWith("http://")||input.startsWith("https://")->input;input.contains(" ")->yahooSearch+Uri.encode(input);input.contains(".")->"https://$input";else->yahooSearch+Uri.encode(input)};browser.loadUrl(url)}
    private fun newPrivateSession(){browser.stopLoading();browser.clearHistory();browser.clearCache(true);browser.clearFormData();CookieManager.getInstance().removeAllCookies(null);CookieManager.getInstance().flush();address.setText("");title.text="T3 Private Browser • Yahoo";status.text="Private session • cookies disabled";browser.loadUrl(yahooHome)}
