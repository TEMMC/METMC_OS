...
class MetmcLockScreen(context: Context, private val correctPassword: String = "metmc", private val onUnlocked: Runnable) : FrameLayout(context) {
    ...
    init {
        ...
        post {
            ...
            if (MetmcSettingsStore.getLockType(context) != "none") {
                ...
                (context.getSystemService(Context.INPUT_METHOD_SERVICE) as? InputMethodManager)?.showSoftInput(passwordInput, InputMethodManager.SHOW_IMPLICIT)
            }
        }
        post(clockUpdater)
    }
    ...
    private fun setupWallpaper() {
        wallpaperView = ImageView(context).apply {
    ...
    private fun buildUi() {
        ...
        val card = LinearLayout(context).apply {
            ...
            background = rounded(panelColor, dp(20))
            clipChildren = false
        }
        root.addView(card, LinearLayout.LayoutParams(-1, ViewGroup.LayoutParams.WRAP_CONTENT).apply { gravity = Gravity.CENTER_HORIZONTAL; topMargin = dp(8); bottomMargin = dp(8) })
        ...
        passwordInput = EditText(context).apply {
            inputType = when (MetmcSettingsStore.getLockType(context)) {
                "pin" -> InputType.TYPE_CLASS_NUMBER or InputType.TYPE_NUMBER_VARIATION_PASSWORD
        ...
        if (MetmcSettingsStore.getLockType(context) == "none") {
            passwordInput.visibility = GONE
            unlock.visibility = GONE
            status.text = "Lock screen disabled"
            postDelayed({ attemptUnlock() }, 150)
        }
        updateDateTime()
    }
    ...
    private fun attemptUnlock() {
        ...
        if (type == "none" || entered == MetmcSettingsStore.getCredential(context) || (MetmcSettingsStore.getCredential(context).isBlank() && entered == correctPassword)) {
            ...
            status.text = "Incorrect credential"
    ...
    private fun updateMediaInfo() {
        if (::mediaInfo.isInitialized) {
            val t = MetmcMediaInfo.displayText()
            mediaInfo.text = if (MetmcMediaInfo.playing && t.isNotBlank()) "♫ $t" else t
        }
    }
    ...
    private fun updateDateTime() {
        ...
        date.text = SimpleDateFormat("EEEE, d MMMM yyyy", Locale.getDefault()).format(now)
    }
