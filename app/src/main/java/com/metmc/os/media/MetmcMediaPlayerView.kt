...
class MetmcMediaPlayerView(private val context: Context) : LinearLayout(context) {
    ...
    private fun build() {
        val top = LinearLayout(context).apply { orientation = HORIZONTAL; gravity = Gravity.CENTER_VERTICAL; setPadding(dp(14),dp(8),dp(10),dp(8)); setBackgroundColor(Color.rgb(22,25,31)) }
        top.addView(TextView(context).apply { text="♫"; textSize=22f; setTextColor(Color.WHITE); gravity=Gravity.CENTER }, LinearLayout.LayoutParams(dp(42),dp(46)))
        top.addView(TextView(context).apply { text="METMC Player"; textSize=16f; typeface=Typeface.DEFAULT_BOLD; setTextColor(Color.WHITE); gravity=Gravity.CENTER_VERTICAL }, LinearLayout.LayoutParams(0,dp(46),1f))
        top.addView(iconButton("＋") { openPicker() }, buttonParams(50)); top.addView(iconButton("☷") { scanLibrary() }, buttonParams(50)); addView(top,LinearLayout.LayoutParams(-1,dp(62)))
        ...
        dropHint.apply { text="DROP MEDIA HERE"; textSize=15f; typeface=Typeface.DEFAULT_BOLD; setTextColor(Color.rgb(180,185,195)); gravity=Gravity.CENTER; setPadding(dp(20),dp(20),dp(20),dp(20)); setOnDragListener(dragListener()) }
        stage.addView(dropHint,FrameLayout.LayoutParams(-1,-1)); addView(stage,LinearLayout.LayoutParams(-1,0,1f))

        title.apply { text=""; textSize=15f; typeface=Typeface.DEFAULT_BOLD; setTextColor(Color.WHITE); gravity=Gravity.CENTER_VERTICAL; setPadding(dp(16),0,dp(16),0); maxLines=1 }; addView(title,LinearLayout.LayoutParams(-1,dp(42)))
        status.apply { text=""; textSize=11f; setTextColor(Color.LTGRAY); gravity=Gravity.CENTER }; addView(status,LinearLayout.LayoutParams(-1,dp(22)))
        ...
        controls.addView(iconButton("↶"){seekBy(-10000)},controlParams()); play.apply{text="▶";textSize=18f;isAllCaps=false;setTextColor(Color.WHITE);minWidth=0;minimumWidth=0;setOnClickListener{toggle()}}; controls.addView(play,LinearLayout.LayoutParams(0,dp(48),1f)); controls.addView(iconButton("■"){stop()},controlParams()); controls.addView(iconButton("↷"){seekBy(10000)},controlParams()); addView(controls,LinearLayout.LayoutParams(-1,dp(64)))
    ...
    private fun dragListener()=OnDragListener{_,event->when(event.action){DragEvent.ACTION_DRAG_STARTED->event.clipDescription?.hasMimeType("*/*")==true||event.clipDescription?.hasMimeType("text/uri-list")==true;DragEvent.ACTION_DRAG_ENTERED->{dropHint.setTextColor(Color.WHITE);true};DragEvent.ACTION_DRAG_EXITED->{dropHint.setTextColor(Color.rgb(180,185,195));true};DragEvent.ACTION_DROP->{dropHint.setTextColor(Color.rgb(180,185,195));event.clipData?.getItemAt(0)?.uri?.let{openUri(it);true}?:false};else->true}}
    private fun iconButton(label:String,action:()->Unit)=Button(context).apply{text=label;textSize=17f;isAllCaps=false;setTextColor(Color.WHITE);minWidth=0;minimumWidth=0;setPadding(0,0,0,0);setOnClickListener{action()}}
    ...
    private fun showLibrary(files:List<File>){val box=LinearLayout(context).apply{orientation=VERTICAL;setPadding(dp(10),dp(8),dp(10),dp(8))};box.addView(TextView(context).apply{text="Media Library";textSize=17f;typeface=Typeface.DEFAULT_BOLD;setTextColor(Color.WHITE);setPadding(dp(6),dp(6),dp(6),dp(10))});val scroll=ScrollView(context);val list=LinearLayout(context).apply{orientation=VERTICAL};files.forEach{f->list.addView(iconButton("${if(f.extension.lowercase(Locale.ROOT)in videoExt)"▣" else "♫"}  ${f.name}"){openFile(f)},LinearLayout.LayoutParams(-1,dp(50)))};if(files.isEmpty())list.addView(TextView(context).apply{text="No media found";setTextColor(Color.LTGRAY);setPadding(dp(12),dp(20),dp(12),dp(20))});scroll.addView(list);box.addView(scroll,LinearLayout.LayoutParams(-1,0,1f));AlertDialogCompat.show(context,box)}
...
private object AlertDialogCompat{fun show(context:Context,content:View){val d=android.app.Dialog(context);d.setContentView(content);d.show();d.window?.setLayout(minOf(720,context.resources.displayMetrics.widthPixels-32),minOf(520,context.resources.displayMetrics.heightPixels-80))}}
