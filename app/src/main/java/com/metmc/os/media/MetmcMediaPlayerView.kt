...
class MetmcMediaPlayerView(private val context: Context) : LinearLayout(context) {
    ...
    private fun build() {
        ...
        controls.addView(play, LinearLayout.LayoutParams(0, dp(46), 1f))
        controls.addView(button("Stop") { stop() }, controlParams())
        controls.addView(button("+10s") { seekBy(10000) }, controlParams())
    ...
    fun openUri(uri: Uri) {
        ...
        try {
            ...
            player = media
            media.setOnPreparedListener { prepared -> status.text = format(prepared.duration); play.text = "❚❚ Pause"; prepared.start(); updateProgress() }
            media.setOnCompletionListener { play.text = "▶ Play"; seek.progress = 0 }
            media.setOnErrorListener { _, _, _ -> status.text = "This media format could not be played"; true }
            media.prepareAsync()
        } catch (e: Exception) { status.text = "Cannot open media: ${e.message}" }
    }

    private fun scanLibrary() {
        status.text = "Scanning media…"
        Thread {
            ...
            fun walk(file: File, depth: Int) {
                ...
                val children = try { file.listFiles() } catch (_: Exception) { null } ?: return
                children.forEach { child -> if (child.isDirectory) walk(child, depth + 1) else if (child.extension.lowercase(Locale.ROOT) in mediaExt) files.add(child) }
            }
    ...
    private fun showLibrary(files: List<File>) {
        ...
        AlertDialogCompat.show(context, box)
    }

    private fun openFile(file: File) {
        if (!file.exists()) { status.text = "File is unavailable"; return }
        release()
        title.text = file.name
        if (file.extension.lowercase(Locale.ROOT) in videoExt) {
            video.visibility = VISIBLE
            video.setVideoPath(file.absolutePath)
            video.setOnPreparedListener { media -> player = media; status.text = format(media.duration); play.text = "❚❚ Pause"; media.start(); updateProgress() }
            video.setOnCompletionListener { play.text = "▶ Play" }
            video.setOnErrorListener { _, _, _ -> status.text = "Video format could not be played"; true }
        } else {
            video.visibility = GONE
            try {
                val media = android.media.MediaPlayer()
                ...
                media.setOnPreparedListener { prepared -> status.text = format(prepared.duration); play.text = "❚❚ Pause"; prepared.start(); updateProgress() }
                media.setOnCompletionListener { play.text = "▶ Play" }
                media.setOnErrorListener { _, _, _ -> status.text = "Audio format could not be played"; true }
                media.prepareAsync()
            } catch (e: Exception) { status.text = "Cannot open media: ${e.message}" }
        }
    }
    ...
    private fun updateProgress() { val media = player ?: return; if (!media.isPlaying) return; if (media.duration > 0) seek.progress = media.currentPosition * 1000 / media.duration; handler.postDelayed({ updateProgress() }, 500) }
...
private object AlertDialogCompat {
    fun show(context: Context, content: android.view.View) {
        val dialog = android.app.Dialog(context)
        dialog.setContentView(content)
        dialog.show()
        dialog.window?.setLayout((600 * context.resources.displayMetrics.density).toInt(), (460 * context.resources.displayMetrics.density).toInt())
    }
}
