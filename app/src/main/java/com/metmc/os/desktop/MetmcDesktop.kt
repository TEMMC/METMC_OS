...
class MetmcDesktop(context: Context) : FrameLayout(context) {
    ...
    private fun createWindow(title: String, content: View): View {
        ...
        val aw = (if (desktopArea.width > 0) desktopArea.width else resources.displayMetrics.widthPixels).coerceAtLeast(1)
        val ah = (if (desktopArea.height > 0) desktopArea.height else resources.displayMetrics.heightPixels - dp(58)).coerceAtLeast(1)
        val mx = (aw - w).coerceAtLeast(0); val my = (ah - h).coerceAtLeast(0); val c = windows.size * dp(28)
    ...
    fun showLauncher() {
        ...
        box.addView(TextView(context).apply { text = "Applications"; textSize = 20f; typeface = android.graphics.Typeface.DEFAULT_BOLD; setTextColor(Color.WHITE); setPadding(dp(4), dp(4), dp(4), dp(12)) })
    ...
    fun openWallpaperPicker() {
        activity?.let { val intent = Intent(Intent.ACTION_OPEN_DOCUMENT).apply { type = "image/*"; addCategory(Intent.CATEGORY_OPENABLE); addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION or Intent.FLAG_GRANT_PERSISTABLE_URI_PERMISSION) }; it.startActivityForResult(intent, 9001) }
    }
