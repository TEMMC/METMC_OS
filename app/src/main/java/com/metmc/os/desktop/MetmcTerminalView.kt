...
class MetmcTerminalView(private val context: android.content.Context) : LinearLayout(context) {
    ...
    private val prompt="metmc@debian:~$ "; private val rootfs="/data/local/linux/rootfs"
    init{
        ...
        input.apply{hint="enter command";setHintTextColor(Color.rgb(100,105,112));setTextColor(Color.WHITE);textSize=14f;typeface=Typeface.MONOSPACE;setSingleLine(true);background=null;imeOptions=EditorInfo.IME_ACTION_DONE};bar.addView(input,LinearLayout.LayoutParams(0,dp(50),1f));addView(bar,LinearLayout.LayoutParams(-1,dp(58)))
    ...
    private fun execute(command:String){val cmd=command.trim();input.setText("");if(cmd.isBlank()){append(prompt);return};if(history.lastOrNull()!=cmd)history.add(cmd);historyIndex=history.size;append("$prompt$cmd\n");if(cmd=="clear"){output.text=prompt;return};if(cmd=="exit"){append("exit is disabled for the desktop shell.\n$prompt");return};Thread{try{val bridge="mkdir -p '$rootfs/storage/emulated/0'; mountpoint -q '$rootfs/storage/emulated/0' || mount --bind /storage/emulated/0 '$rootfs/storage/emulated/0' 2>/dev/null || true; cd /home 2>/dev/null || cd /root 2>/dev/null || cd /; export HOME=/home; export USER=metmc; export LOGNAME=metmc; export PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin; export DISPLAY=:100; exec /bin/bash -lc ${q(cmd)}";val p=ProcessBuilder("su","-c","chroot '$rootfs' /bin/bash -c ${q(bridge)}").redirectErrorStream(true).start();val r=BufferedReader(InputStreamReader(p.inputStream));val result=StringBuilder();var line:String?;while(r.readLine().also{line=it}!=null)result.append(line).append('\n');val code=p.waitFor();(context as?Activity)?.runOnUiThread{append(result.toString());if(code!=0)append("[exit $code]\n");append(prompt)}}catch(e:Exception){(context as?Activity)?.runOnUiThread{append("Terminal error: ${e.message}\n$prompt")}}}.start()}
    private fun append(v:String){output.append(v);(output.parent as?ScrollView)?.post{(output.parent as ScrollView).fullScroll(ScrollView.FOCUS_DOWN)}}
    private fun q(v:String)="'"+v.replace("'","'\\''")+"'";private fun dp(v:Int)=(v*resources.displayMetrics.density).toInt()
}
