...
class MetmcTerminalView(private val context: android.content.Context) : LinearLayout(context) {
    ...
    init {
        ...
        top.addView(Button(context).apply {
            text = "Clear"
            isAllCaps = false
            setOnClickListener { output.text = ""; append(prompt) }
    ...
    private fun execute(command: String) {
        ...
        append("$prompt$cmd\n")
        if (cmd == "clear") { output.text = ""; append(prompt); return }
        ...
        Thread {
            try {
                ...
                val process = ProcessBuilder("su", "-c", "chroot '$rootfs' /bin/bash -c ${quote(bridge)}")
