...
class MetmcTerminalView(private val context: android.content.Context) : LinearLayout(context) {
    ...
    init {
        ...
        top.addView(Button(context).apply {
            text = "Clear"
            isAllCaps = false
            setOnClickListener { output.text = prompt }
        ...
        bar.addView(TextView(context).apply {
            text = prompt
            setTextColor(Color.rgb(110, 205, 135))
            textSize = 13f
            typeface = Typeface.MONOSPACE
        }, LinearLayout.LayoutParams(-2, dp(50)))
        input.apply {
            hint = "enter command"
            setHintTextColor(Color.rgb(100, 105, 112))
    ...
    private fun execute(command: String) {
        ...
        if (cmd == "exit") {
            append("exit is disabled for the desktop shell.\n$prompt")
            return
        }
        Thread {
            try {
                val bridge = "mkdir -p '$rootfs/storage/emulated/0'; mountpoint -q '$rootfs/storage/emulated/0' || mount --bind /storage/emulated/0 '$rootfs/storage/emulated/0' 2>/dev/null || true; cd /home 2>/dev/null || cd /root 2>/dev/null || cd /; export HOME=/home; export USER=metmc; export LOGNAME=metmc; export PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin; export DISPLAY=:100; exec /bin/bash -lc ${q(cmd)}"
                val p = ProcessBuilder("su", "-c", "chroot '$rootfs' /bin/bash -c ${q(bridge)}")
                ...
                (context as? Activity)?.runOnUiThread {
                    append("Terminal error: ${e.message}\n$prompt")
                }
            }
        }.start()
    }
