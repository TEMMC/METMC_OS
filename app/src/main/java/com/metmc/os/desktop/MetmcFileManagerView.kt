...
class MetmcFileManagerView(private val context: Context) : LinearLayout(context) {
    ...
    private fun buildToolbar() {
        ...
        bar.setBackgroundColor(Color.rgb(24, 28, 37))
    ...
    private fun buildTabs() {
        ...
        row.setBackgroundColor(Color.rgb(18, 21, 28))
        val labels = arrayOf("Internal", "SD Card", "Linux", "ROOT")
    ...
    private fun refresh() {
        ...
        when (tab) {
            ...
            Tab.SD -> if (sdCurrent != null) loadAndroidPath(sdCurrent!!) else showMessage("No removable SD card detected.")
    ...
    private fun loadAndroidPath(target: String) {
        ...
        Thread {
            val q = quote(target)
            val script = "target=$q; for x in \"\${'$'}target\"/* \"\${'$'}target\"/.[!.]* \"\${'$'}target\"/..?*; do [ -e \"\${'$'}x\" ] || continue; if [ -d \"\${'$'}x\" ]; then printf 'd\\t%s\\t0\\n' \"\${'$'}x\"; else size=\$(stat -c %s \"\${'$'}x\" 2>/dev/null || echo 0); printf 'f\\t%s\\t%s\\n' \"\${'$'}x\" \"\${'$'}size\"; fi; done"
            val output = runRoot("sh -c ${quote(script)}")
            (context as? Activity)?.runOnUiThread {
                list.removeAllViews()
                if (output.startsWith("ERROR")) showMessage(output.trim()) else renderRows(output)
            }
        }.start()
    }
    ...
    private fun loadLinux(target: String) {
        ...
        Thread {
            val q = quote(target)
            val script = "for x in \${q}/* \${q}/.[!.]* \${q}/..?*; do [ -e \"\${'$'}x\" ] || continue; if [ -d \"\${'$'}x\" ]; then printf 'd\\t%s\\t0\\n' \"\${'$'}x\"; else size=\$(stat -c %s \"\${'$'}x\" 2>/dev/null || echo 0); printf 'f\\t%s\\t%s\\n' \"\${'$'}x\" \"\${'$'}size\"; fi; done"
    ...
    private fun renderRows(raw: String) {
        ...
        rows.forEach { row ->
            ...
            addRow(if (type == "d") "▰" else "□", name, if (type == "d") "Folder" else formatSize(bytes)) { if (type == "d") enter(full) else open(full) }
        }
    }
    ...
    private fun addRow(icon: String, name: String, detail: String, action: () -> Unit) {
        ...
        row.addView(iconView, LinearLayout.LayoutParams(dp(44), dp(58)))
        ...
        detailView.setTextColor(Color.rgb(145, 152, 166))
    ...
    private fun showMessage(message: String) {
        ...
        list.addView(view, LinearLayout.LayoutParams(-1, -2))
    }
    ...
    private fun up() {
        when (tab) {
            Tab.INTERNAL -> if (internalPath != "/storage/emulated/0") internalPath = parentPath(internalPath, "/storage/emulated/0")
            Tab.SD -> { val base = sdPath; val current = sdCurrent; if (base != null && current != null && current != base) sdCurrent = parentPath(current, base) }
            Tab.LINUX -> if (linuxPath != "/") linuxPath = parentPath(linuxPath, "/")
            Tab.ROOT -> if (rootPath != "/") rootPath = parentPath(rootPath, "/")
        }
        refresh()
    }

    private fun parentPath(value: String, root: String): String {
        val clean = value.trimEnd('/')
        val slash = clean.lastIndexOf('/')
        val parent = if (slash <= 0) "/" else clean.substring(0, slash)
    ...
    private fun newFolder() {
        ...
        AlertDialog.Builder(context).setTitle("New folder").setView(input).setNegativeButton("Cancel", null).setPositiveButton("Create") { _, _ ->
            ...
            val target = "$base/$name"
            val command = if (tab == Tab.LINUX) "chroot /data/local/linux/rootfs /bin/mkdir -p ${quote(target)}" else "mkdir -p ${quote(target)}"
            Thread {
                runRoot(command)
    ...
    private fun open(full: String) {
        try {
            val file = File(full)
            if (!file.exists()) { Toast.makeText(context, "File is not directly readable by Android", Toast.LENGTH_SHORT).show(); return }
    ...
    private fun formatSize(value: String): String {
        ...
        return when {
            number >= 1073741824L -> String.format(Locale.US, "%.1f GB", number / 1073741824.0)
            number >= 1048576L -> String.format(Locale.US, "%.1f MB", number / 1048576.0)
            number >= 1024L -> String.format(Locale.US, "%.1f KB", number / 1024.0)
            else -> "$number B"
        }
    }
