...
class MetmcFileManagerView(private val context: Context) : LinearLayout(context) {
    ...
    private fun detectSdAndRefresh(){Thread{val out=runRoot("for d in /storage/*; do [ -d \"$d\" ] && case \"$d\" in /storage/emulated|/storage/self) ;; *) echo \"$d\";; esac; done 2>/dev/null");val candidates=out.lines().map{it.trim()}.filter{it.startsWith("/storage/")};sdPath=candidates.firstOrNull();sdCurrent=sdPath;(context as?Activity)?.runOnUiThread{redrawTabs();refresh()}}.start()}
    ...
    private fun loadAndroidPath(target:String){show("Loading…");Thread{val script="target=${quote(target)}; for x in \"$target\"/* \"$target\"/.[!.]* \"$target\"/..?*; do [ -e \"$x\" ] || continue; if [ -d \"$x\" ]; then printf 'd\\t%s\\t0\\n' \"$x\"; else s=$(stat -c %s \"$x\" 2>/dev/null || echo 0); printf 'f\\t%s\\t%s\\n' \"$x\" \"$s\"; fi; done";val out=runRoot("sh -c ${quote(script)}");(context as?Activity)?.runOnUiThread{list.removeAllViews();if(out.startsWith("ERROR"))show(out.trim())else renderFind(out)}}.start()}
    private fun loadLinux(target:String){show("Loading Debian…");Thread{val script="for x in \"$target\"/* \"$target\"/.[!.]* \"$target\"/..?*; do [ -e \"$x\" ] || continue; if [ -d \"$x\" ]; then printf 'd\\t%s\\t0\\n' \"$x\"; else s=$(stat -c %s \"$x\" 2>/dev/null || echo 0); printf 'f\\t%s\\t%s\\n' \"$x\" \"$s\"; fi; done";val cmd="chroot /data/local/linux/rootfs /bin/sh -c ${quote(script)}";val out=runRoot(cmd);(context as?Activity)?.runOnUiThread{list.removeAllViews();if(out.startsWith("ERROR"))show(out.trim())else renderFind(out)}}.start()}
    private fun renderFind(raw:String){val rows=raw.lines().mapNotNull{val p=it.split('\t',limit=3);if(p.size<3)null else Triple(p[0],p[1],p[2])}.sortedWith(compareByDescending<Triple<String,String,String>>{it.first=="d"}.thenBy{it.second.substringAfterLast('/').lowercase(Locale.ROOT)});if(rows.isEmpty()){show("This folder is empty.");return};rows.forEach{(type,full,bytes)->val name=full.trimEnd('/').substringAfterLast('/');addRow(if(type=="d")"▰"else"□",name,if(type=="d")"Folder" else formatSize(bytes)){if(type=="d")enter(full)else open(full)}}}
