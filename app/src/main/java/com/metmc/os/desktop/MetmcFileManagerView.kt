...
class MetmcFileManagerView(private val context: Context) : LinearLayout(context) {
    ...
    private fun tabBar(){val row=LinearLayout(context).apply{orientation=HORIZONTAL;setBackgroundColor(Color.rgb(18,21,28))};val labels=arrayOf("Internal","SD Card","Linux","ROOT");labels.forEachIndexed{i,label->val b=Button(context);b.text=label;b.isAllCaps=false;b.textSize=11f;b.setOnClickListener{tab=Tab.entries[i];redrawTabs();refresh()};tabs.add(b);row.addView(b,LayoutParams(0,dp(46),1f))};addView(row,LayoutParams(-1,dp(46)));redrawTabs()}
    private fun redrawTabs(){tabs.forEachIndexed{i,b->val selected=Tab.entries[i]==tab;b.setTextColor(if(selected)Color.WHITE else Color.rgb(145,152,166));b.setBackgroundColor(if(selected)Color.rgb(50,57,72) else Color.rgb(18,21,28));if(i==1)b.isEnabled=sdPath!=null}}
    ...
    private fun detectSdAndRefresh(){Thread{val out=runRoot("find /storage -mindepth 1 -maxdepth 1 -type d -print 2>/dev/null");val candidates=out.lines().map{it.trim()}.filter{it.startsWith("/storage/")&&!it.startsWith("/storage/emulated")&&!it.endsWith("self")};sdPath=candidates.firstOrNull();(context as?Activity)?.runOnUiThread{redrawTabs();refresh()}}.start()}
    private fun refresh(){path.text=when(tab){Tab.INTERNAL->internalPath;Tab.SD->sdPath?:("/storage (no SD card detected)");Tab.LINUX->"LINUX:$linuxPath";Tab.ROOT->"ROOT:$rootPath"};list.removeAllViews();when(tab){Tab.INTERNAL->loadAndroidPath(internalPath);Tab.SD->sdPath?.let{loadAndroidPath(if(sdPath==it&&internalPath=="__none__")it:currentSdPath(it))}?:show("No removable SD card was detected.");Tab.LINUX->loadLinux(linuxPath);Tab.ROOT->loadAndroidPath(rootPath)}}
    private var sdCurrent:String?=null
    private fun currentSdPath(base:String):String{if(sdCurrent==null)sdCurrent=base;return sdCurrent!!}
    private fun loadAndroidPath(target:String){show("Loading…");Thread{val out=runRoot("find ${quote(target)} -mindepth 1 -maxdepth 1 -printf '%y\\t%p\\t%s\\n' 2>&1");(context as?Activity)?.runOnUiThread{list.removeAllViews();if(out.contains("Permission denied")||out.startsWith("ERROR"))show(out.trim())else renderFind(out,target)}}.start()}
    private fun loadLinux(target:String){show("Loading Debian…");Thread{val cmd="chroot /data/local/linux/rootfs /bin/sh -c ${quote("find ${quote(target)} -mindepth 1 -maxdepth 1 -printf '%y\\t%p\\t%s\\n' 2>&1")}";val out=runRoot(cmd);(context as?Activity)?.runOnUiThread{list.removeAllViews();if(out.contains("Permission denied")||out.startsWith("ERROR"))show(out.trim())else renderFind(out,target)}}.start()}
    private fun renderFind(raw:String,target:String){val rows=raw.lines().mapNotNull{val p=it.split('\t');if(p.size<3)null else Triple(p[0],p[1],p[2])}.sortedWith(compareByDescending<Triple<String,String,String>>{it.first=="d"}.thenBy{it.second.lowercase(Locale.ROOT)});if(rows.isEmpty()){show("This folder is empty.");return};rows.forEach{(type,full,size)->val name=full.substringAfterLast('/');addRow(if(type=="d")"▰"else"□",name,if(type=="d")"Folder" else size(size)){if(type=="d")enter(full)else open(full)}}}
    ...
    private fun home(){when(tab){Tab.INTERNAL->internalPath="/storage/emulated/0";Tab.SD->sdPath?.let{sdCurrent=it};Tab.LINUX->linuxPath="/";Tab.ROOT->rootPath="/"};refresh()}
    ...
    private fun parent(p:String,root:String):String{val q=p.trimEnd('/');val x=q.substringBeforeLast('/','/');return if(x.length<root.length)root.ifBlank{"/"} else x.ifBlank{"/"}}
    private fun newFolder(){val input=EditText(context).apply{hint="Folder name"};AlertDialog.Builder(context).setTitle("New folder").setView(input).setNegativeButton("Cancel",null).setPositiveButton("Create"){_,_->val name=input.text.toString().trim();if(name.isBlank()||name.contains('/')||name=="."||name==".."){Toast.makeText(context,"Invalid folder name",Toast.LENGTH_SHORT).show();return@setPositiveButton};val base=when(tab){Tab.INTERNAL->internalPath;Tab.SD->sdCurrent;Tab.LINUX->"$linuxPath";Tab.ROOT->rootPath};if(base==null){Toast.makeText(context,"No SD card",Toast.LENGTH_SHORT).show();return@setPositiveButton};val target=if(tab==Tab.LINUX)"chroot /data/local/linux/rootfs /bin/mkdir -p ${quote("$base/$name")}" else "mkdir -p ${quote("$base/$name")}";Thread{val r=runRoot(target);(context as?Activity)?.runOnUiThread{if(r.startsWith("ERROR"))Toast.makeText(context,r,Toast.LENGTH_LONG).show();refresh()}}}.show()}
    private fun open(full:String){try{val file=File(full);if(file.exists()&&!full.startsWith("/data/local/linux")){context.startActivity(Intent(Intent.ACTION_VIEW).apply{setDataAndType(Uri.fromFile(file),"*/*");addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION)})}else Toast.makeText(context,"Use an Android app or Linux application to open this file",Toast.LENGTH_SHORT).show()}catch(_:Exception){Toast.makeText(context,"No application can open this file",Toast.LENGTH_SHORT).show()}}
    ...
    private fun size(v:String):String{val n=v.toLongOrNull()?:0;return when{n>=1073741824->String.format(Locale.US,"%.1f GB",n/1073741824.0);n>=1048576->String.format(Locale.US,"%.1f MB",n/1048576.0);n>=1024->String.format(Locale.US,"%.1f KB",n/1024.0);else->"$n B"}}
}
