...
class SystemAppRemoverActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        content.addView(PremiumUi.text(
            this,
            "Only changes made by METMC OS are restored.",
            12f,
            PremiumUi.muted
    ...
    private fun restoreAll() {
        ...
        if (changed == 0) {
            status.text = "No METMC OS changes to restore"
            return
        }
        MaterialAlertDialogBuilder(this)
    ...
    private fun runAction(packages: List<String>, disable: Boolean) {
        ...
        Thread({
            val result = runCatching {
                ...
                if (disable) manager.disable(packages) else manager.restoreAllChangedByMETMC OS()
            }
            runOnUiThread {
