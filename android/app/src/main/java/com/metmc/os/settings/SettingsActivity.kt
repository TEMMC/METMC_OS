...
import com.metmc.os.launcher.KioskActivity
...
class SettingsActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        content.addView(PremiumUi.sectionLabel(this, "METMC OS mode"))
        content.addView(group().apply {
            desktopCheck = PremiumUi.text(this@SettingsActivity, "", 20f, PremiumUi.primary)
            addView(row(
                title = "Linux desktop",
                subtitle = "Open METMC OS like a normal app",
                trailing = desktopCheck
            ) {
                METMCPreferences.setOperatingMode(this@SettingsActivity, METMCPreferences.OperatingMode.DESKTOP_APP)
            ...
            addView(row(
                title = "Home launcher",
                subtitle = "Use METMC OS as your phone's Home screen",
                trailing = homeCheck
            ) {
                METMCPreferences.setOperatingMode(this@SettingsActivity, METMCPreferences.OperatingMode.HOME_LAUNCHER)
        ...
        content.addView(group().apply {
            addView(switchRow(
                ...
                METMCPreferences.hideSystemBars(this@SettingsActivity)
            ) { checked ->
                METMCPreferences.setHideSystemBars(this@SettingsActivity, checked)
            })
            addView(PremiumUi.separator(this@SettingsActivity))
            addView(switchRow(
                "Show Android apps",
                "Include installed Android apps in the Linux drawer",
                METMCPreferences.showAndroidApps(this@SettingsActivity)
            ) { checked ->
                METMCPreferences.setShowAndroidApps(this@SettingsActivity, checked)
    ...
    private fun scanGoogleComponents() {
        ...
        }, "metmc-degoogle-scan").start()
    }

    private fun updateState() {
        if (!::summary.isInitialized) return
        val mode = METMCPreferences.operatingMode(this)
        desktopCheck.text = if (mode == METMCPreferences.OperatingMode.DESKTOP_APP) "✓" else ""
        homeCheck.text = if (mode == METMCPreferences.OperatingMode.HOME_LAUNCHER) "✓" else ""
        summary.text = when {
            mode == METMCPreferences.OperatingMode.HOME_LAUNCHER && HomeRoleManager.isDefaultHome(this) ->
                "METMC OS is your Home launcher"
            mode == METMCPreferences.OperatingMode.HOME_LAUNCHER ->
