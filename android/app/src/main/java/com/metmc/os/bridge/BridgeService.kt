 ...
 *
 * The socket is at: /data/data/com.metmc/files/bridge/bridge.sock
 * which is bind-mounted into the chroot at: /run/metmc/bridge.sock
 *
...
class BridgeService : Service() {
    ...
    companion object {
        private const val TAG = "METMC OS.Bridge"
        private const val NOTIFICATION_CHANNEL = "metmc_bridge"
        private const val NOTIFICATION_ID = 1001
        const val ACTION_REFRESH_NOTIFICATION = "com.metmc.os.action.REFRESH_NOTIFICATION"
    }
    ...
    private fun startSocketServer() {
        ...
        thread(name = "bridge-socket-server") {
            try {
                ...
                serverSocket = LocalServerSocket("metmc_bridge")
                Log.i(TAG, "Bridge socket server listening on: metmc_bridge")

                // Also create a filesystem socket via symlink for the chroot
                // The chroot accesses /run/metmc/bridge.sock
    ...
    private fun createNotificationChannel() {
        val channel = NotificationChannel(
            ...
            description = "METMC OS hardware bridge service"
            setShowBadge(false)
        }
