# Bundled METMC OS rootfs

All builds require `metmc-rootfs-arm64.tgz` and its matching
`metmc-rootfs-arm64.tgz.sha256` in this directory. Gradle fails before
...
icons, and the METMC OS compatibility libraries. Do not package a user's live
...
`/usr/share/metmc/rootfs-packages.txt`. Set `SOURCE_DATE_EPOCH` when producing
