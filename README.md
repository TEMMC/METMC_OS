<p align="center">
  <img src="linux.png" width="180" alt="METMC OS logo">
</p>

<h1 align="center">METMC OS</h1>
...
| --- | --- |
| Rooted ARM64 phone | **METMC OS** — Ubuntu chroot, Phosh, embedded X11, Android integration |
...
METMC OS does not provide a non-root mode. If you do not want to root your phone, download DroidDesk from its [Releases page](https://github.com/orailnoor/DroidDesk/releases) and follow its README. The projects share some display technology, but their Linux runtime and capabilities are different.
...
METMC OS is not a replacement ROM. Android still boots and remains underneath Linux. Rooted devices may fail Play Integrity, banking, DRM, or enterprise security checks regardless of METMC OS. Android hardware bridges are experimental and should not be treated as replacements for emergency calling or other critical functions.
...
8. Only then, if desired, open METMC OS Settings and select METMC OS as the default Android launcher.

If Android reports a signature conflict while updating an older development build, uninstall that build first. Uninstalling METMC OS erases its Linux root filesystem and installed Linux applications, so copy important files to the shared folder before doing this.

## Basic controls

- Tap the floating three-dot button to open the METMC OS menu.
...
- METMC OS Settings can also be opened from the ongoing Android notification.
...
Android's Files app may also show a **METMC OS Shared** storage provider. Sync is not instantaneous; keep METMC OS running and allow a few seconds for large files. Avoid editing the same file simultaneously from Android and Linux.

## Installing Linux applications

METMC OS is ARM64 Linux. An application must provide an `aarch64`/`arm64` build and be compatible with the nested display and Android kernel environment. Being listed on Flathub or offered as a `.deb` does not guarantee ARM64 support.
...
These applications run outside Flatpak's sandbox. They generally integrate more predictably, but they have full access to the METMC OS Linux environment.

### Local `.deb` files

Place the `.deb` in the Android METMC OS shared folder, then verify it in Linux:
...
When OpenSSH is installed, METMC OS starts it on port `8022` with password login disabled. Add your public key to `/root/.ssh/authorized_keys`, then connect using the phone's LAN address:
...
### METMC OS says root is unavailable

- Open your root manager and grant METMC OS permanent access.
- Confirm that `su` works from ADB or another trusted root shell.
- Reopen METMC OS after changing the root decision.
...
- Force-stop METMC OS and open it again.
- Disable aggressive battery restrictions for METMC OS.
- If it repeats, capture logs instead of repeatedly clearing app data.

### Returning from an Android app shows black

Open the floating menu and use Home. If the surface does not recover, switch away from METMC OS and return once, then collect logs. Include whether the problem happens only in portrait, landscape, or after a particular Android application.
...
```bash
adb logcat -c
adb logcat | grep -iE 'metmc|xlorie|phoc|phosh|flatpak|bwrap|mesa|vulkan'
```

Include the phone model, Android version, ROM, root manager and version, METMC OS release, GPU, exact reproduction steps, screenshot, and terminal output. Remove personal information before posting logs.

Report reproducible problems through [GitHub Issues](https://github.com/orailnoor/METMC OS/issues).
...
- Setting METMC OS as the default launcher is optional and recovery should be tested first.
...
- An ARM64 rootfs prepared with Phosh, Phoc, GNOME Console, required schemas, icons, and METMC OS compatibility libraries
...
scripts/build-rootfs-asset.sh /path/to/prepared-rootfs \
  android/app/src/main/assets/rootfs/metmc-rootfs-arm64.tgz
```
...
METMC OS executes commands as root to create mounts and run the chroot. Only install APKs from this repository, review system-app removal suggestions carefully, and never share signing keys, root-manager databases, or private logs. METMC OS disables selected system packages rather than promising that every package is safe to delete.

## Credits

Created by [orailnoor](https://youtube.com/@orailnoor).

METMC OS incorporates modified components from [Termux:X11](https://github.com/termux/termux-x11) and builds on architectural work from [DroidDesk](https://github.com/orailnoor/DroidDesk). It is an independent project and is not affiliated with or endorsed by Termux, GNOME, Ubuntu, Canonical, Flathub, or device manufacturers.
